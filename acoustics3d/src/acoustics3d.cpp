...
DiffBEM::DiffBEM(int cluster_size, double radius_factor, const std::vector<double> &freq_bands, int n_freqs, double approx_ACA_tol, double Q_ACA_tol, double solver_tol, double listener_ds, bool recompute_matrices) {
...
std::pair<double, bem3d::vec> DiffBEM::gradient(const bem3d::vec &x) {
    ...
    d_grad.setZero(_HH);
    for (double freq_band : _freq_bands) {
        c += bem3d::gradient(_Ne, _HH, _elements, Ps, _Es, Cs, Ns, _Ls, false, freq_band, _n_freqs, false, xs, ys, lmbs, adj_bs, dcdh, _approx_ACA_tol, _Q_ACA_tol, _solver_tol, _recompute_matrices, silent, &_direct, &_approx);
        d_grad += dcdh;
    }
...
NB_MODULE(bemlib3d, m) {
    nb::class_<DiffBEM>(m, "DiffBEM")
        .def(nb::init<int, double, const std::vector<double> &, int, double, double, double, double, bool>(),
             ...
             "approx_ACA_tol"_a = 1e-3, "Q_ACA_tol"_a = 1e-3, "solver_tol"_a = 1e-3, "listener_ds"_a = 5, "recompute_matrices"_a = false)
