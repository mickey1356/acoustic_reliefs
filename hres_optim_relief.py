...
def main(folder):
    ...
    init_heights = H.read_image("t_heights2.png", w=hfield_res, h=hfield_res, format="L")
    init_heights = (init_heights - np.min(init_heights)) / (np.max(init_heights) - np.min(init_heights)) * 2 * vmax - vmax
    init_heights = torch.from_numpy(init_heights).to(DEVICE)
    ...
    for it in pbar:
        ...
        tracker_dict["ht_v"] = ht_v.item()
        tracker_dict["ht_g"] = ht_g.detach().cpu().numpy()
...
if __name__ == "__main__":
    fdrs = [
        ...
        "outputs/ptlight_flowers_relief_init"
    ]
